#!/usr/bin/env python3
"""
NCAA Women's Lacrosse Roster Scraper - Unified Architecture
Adapted from women's soccer scraper for lacrosse-specific needs
...
Usage:
    ...
    python src/wlacrosse_roster_scraper.py --season 2025 --limit 5
"""
...
class URLBuilder:
    ...
    def build_roster_url(base_url: str, season: str, url_format: str = 'default') -> str:
        ...
        Args:
            base_url: Team's URL from teams.csv (e.g., 'https://goheels.com/sports/womens-lacrosse')
            season: Season string (e.g., '2024-25')
            url_format: URL pattern type
                - 'default': {base_url}/roster/{season}
                - 'wsoc': {base_url}/2024-25/roster (or /roster/2024)
                - 'sports_wsoc': {base_url}/sports/wlax/{season}/roster
        ...
        if url_format == 'default':
            ...
            # Example: https://ualbanysports.com/sports/womens-lacrosse/roster/2025
            return f"{base_url}/roster/{season}"

        elif url_format == 'wlax_index':
            # wlax/index pattern (11.9% of teams - 98 teams)
            # teams.csv has /sports/wlax/index, need /sports/wlax/roster/2025
            # Example: https://aicyellowjackets.com/sports/wlax/roster/2025
        ...
        elif url_format == 'wlax_plain':
            # /sports/wlax without /index (e.g., Rollins)
            # Example: https://rollinssports.com/sports/wlax/roster/2025
            return f"{base_url}/roster/{season}"
            ...
            # Example: https://ukathletics.com/sports/wlax/roster/season/2025/
            try:
        ...
        elif url_format == 'wlax_season_range':
            # /sports/wlax/{season-range}/roster format (used by some schools like Emory & Henry)
            # Example: https://gowasps.com/sports/wlax/2025-26/roster
            # Convert single year to range (2025 -> 2025-26)
...
class TeamConfig:
    ...
    def get_url_format(cls, team_id: int, team_url: str = '') -> str:
        ...
        if team_url:
            if '/sports/wlax/index' in team_url or '/Sports/wsoc' in team_url:
                return 'wlax_index'
            elif '/sports/wlax' in team_url:
                # /sports/wlax without /index (e.g., Rollins)
                return 'wlax_plain'
            elif '/sports/womens-lacrosse' in team_url or '/sports/wlax' in team_url:
                return 'default'
...
class RosterManager:
    ...
    def save_results(self, players: List[Player], division: Optional[str] = None):
        ...
        json_file = self.output_dir / 'json' / f'rosters_wlax_{self.season}{div_suffix}.json'
        csv_file = self.output_dir / 'csv' / f'rosters_wlax_{self.season}{div_suffix}.csv'
    ...
    def _save_error_reports(self, division: Optional[str] = None):
        ...
        if self.zero_player_teams:
            zero_file = self.output_dir / 'csv' / f'rosters_wlax_{self.season}{div_suffix}_zero_players.csv'
        ...
        if self.failed_teams:
            failed_file = self.output_dir / 'csv' / f'rosters_wlax_{self.season}{div_suffix}_failed.csv'
...
Examples:
  ...
  python src/wlacrosse_roster_scraper.py --division I --season 2025

  # Scrape first 10 Division II teams (testing)
  python src/wlacrosse_roster_scraper.py --division II --limit 10 --season 2025

  # Scrape all teams
  python src/wlacrosse_roster_scraper.py --season 2025

  # Scrape specific team
  python src/wlacrosse_roster_scraper.py --team 14 --season 2025
        """
    )
