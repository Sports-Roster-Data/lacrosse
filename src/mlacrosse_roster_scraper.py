#!/usr/bin/env python3
"""
NCAA Men's Lacrosse Roster Scraper - Unified Architecture
Adapted from women's lacrosse scraper for men's lacrosse

Usage:
    python src/mlacrosse_roster_scraper.py --season 2025 --division I
    python src/mlacrosse_roster_scraper.py --team 593904 --season 2025
    python src/mlacrosse_roster_scraper.py --season 2025 --limit 5
"""
...
@dataclass
class Player:
    """Player data structure for NCAA men's lacrosse rosters"""
    team_id: int
    team: str
    season: str
    division: str = ""
    ...
    major: str = ""  # Lacrosse rosters often include major
    hometown: str = ""
    high_school: str = ""
    previous_school: str = ""
    url: str = ""
...
class FieldExtractors:
    ...
    def extract_position(text: str) -> str:
        """
        Extract position from text - MEN'S LACROSSE VERSION

        Men's lacrosse positions:
        - G (Goalie/Goalkeeper)
        - D (Defense/Defender)
        - M (Midfield/Midfielder)
        - A (Attack/Attacker)
        ...
        # Men's lacrosse specific positions
        position_match = re.search(
            r'\b(SSDM|'  # Short Stick Defensive Midfielder (check first - most specific)
            r'LSM|'  # Long Stick Midfielder
            r'FO|FOGO|'  # Faceoff specialist
            r'GK|G|GOALIE|GOALKEEPER|'  # Goalie variations
            r'DEF|D|DEFENSE|DEFENDER|'  # Defense variations
            r'MID|M|MIDFIELD|MIDFIELDER|'  # Midfield variations
            r'ATT|A|ATTACK|ATTACKER)\b',  # Attack variations
            text,
            re.IGNORECASE
        )
        if position_match:
            ...
            # Normalize variations to standard positions
            # Goalie
            if pos in ('GK', 'G', 'GOALIE', 'GOALKEEPER'):
                return 'G'
            # Defense
            elif pos in ('DEF', 'D', 'DEFENSE', 'DEFENDER'):
                return 'D'
            # Midfield (standard)
            elif pos in ('MID', 'M', 'MIDFIELD', 'MIDFIELDER'):
                return 'M'
            # Attack
            elif pos in ('ATT', 'A', 'ATTACK', 'ATTACKER'):
                return 'A'
            # LSM (Long Stick Midfielder)
            elif pos in ('LSM',):
                return 'LSM'
        ...
        if 'GOALKEEPER' in text_upper or 'GOALIE' in text_upper:
            return 'G'
        elif 'DEFENSE' in text_upper or 'DEFENDER' in text_upper:
            return 'D'
        elif 'LONG STICK' in text_upper and 'MID' in text_upper:
            return 'LSM'
        elif 'SHORT STICK' in text_upper:
            return 'SSDM'
        elif 'FACEOFF' in text_upper or 'FACE-OFF' in text_upper:
            return 'FO'
        elif 'MIDFIELDER' in text_upper or 'MIDFIELD' in text_upper:
            return 'M'
        elif 'ATTACK' in text_upper or 'ATTACKER' in text_upper:
            return 'A'
...
class URLBuilder:
    ...
    def build_roster_url(base_url: str, season: str, url_format: str = 'default') -> str:
        ...
        Args:
            base_url: Team's URL from teams.csv (e.g., 'https://goheels.com/sports/mens-lacrosse')
            season: Season string (e.g., '2024-25')
            url_format: URL pattern type
                - 'default': {base_url}/roster/{season}
                - 'mlax': {base_url}/2024-25/roster (or /roster/2024)
                - 'sports_mlax': {base_url}/sports/mlax/{season}/roster
        ...
        if url_format == 'default':
            ...
            # Example: https://ualbanysports.com/sports/mens-lacrosse/roster/2025
            return f"{base_url}/roster/{season}"

        elif url_format == 'mlax_index':
            # mlax/index pattern (11.9% of teams - 98 teams)
            # teams.csv has /sports/mlax/index, need /sports/mlax/roster/2025
            # Example: https://aicyellowjackets.com/sports/mlax/roster/2025
        ...
        elif url_format == 'mlax_plain':
            # /sports/mlax without /index (e.g., Rollins)
            # Example: https://rollinssports.com/sports/mlax/roster/2025
            return f"{base_url}/roster/{season}"
            ...
            # Example: https://virginiasports.com/sports/mlax/roster/season/2025-26/
            ...
            # Example: https://clemsontigers.com/sports/womens-lacrosse/roster/season/2025/
            try:
            ...
            # Example: https://ukathletics.com/sports/mlax/roster/season/2025/
            try:
        ...
        elif url_format == 'mlax_season_range':
            # /sports/mlax/{season-range}/roster format (used by some schools like Emory & Henry)
            # Example: https://gowasps.com/sports/mlax/2025-26/roster
            # Convert single year to range (2025 -> 2025-26)
    ...
    def extract_base_url(full_url: str) -> str:
        ...
        Example:
            'https://goheels.com/sports/womens-lacrosse' → 'https://goheels.com'
        """
...
class TeamConfig:
    ...
    TEAM_CONFIGS = {
        ...
        186: {'url_format': 'wlax_season_range', 'requires_js': True, 'notes': 'Dist. Columbia - /sports/wlax/YYYY-YY/roster format with JS-rendered player names'},
        8956: {'url_format': 'wlax_season_range', 'requires_js': False, 'notes': 'Dominican (NY) - /sports/wlax/YYYY-YY/roster format'},
    }
    ...
    def get_url_format(cls, team_id: int, team_url: str = '') -> str:
        ...
        Returns:
            URL format string ('default', 'wlax_index', etc.)
        """
        ...
        if team_url:
            if '/sports/mlax/index' in team_url or '/sports/MLA/index' in team_url:
                return 'mlax_index'
            elif '/sports/mlax' in team_url:
                # /sports/mlax without /index
                return 'mlax_plain'
            elif '/sports/mens-lacrosse' in team_url or '/sports/mlax' in team_url:
                return 'default'
...
class StandardScraper:
    ...
    def _build_season_range_url(self, base_url: str, season: str) -> str:
        ...
        # Remove /index suffix if present (e.g., /sports/wlax/index -> /sports/wlax)
...
class JSScraper(StandardScraper):
    ...
    def _build_season_range_url(self, base_url: str, season: str) -> str:
        ...
        - https://ccsubluedevils.com/sports/wlax/2025-26/roster
...
class RosterManager:
    ...
    def load_teams(self, csv_path: str, division: Optional[str] = None) -> List[Dict]:
        ...
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                # Handle different division column formats
                row_division = row.get('ncaa_division_formatted', row.get('division', ''))
                if division is None or row_division == division:
                    # Normalize column names for consistency
                    if 'school_name' in row and 'team' not in row:
                        row['team'] = row['school_name']
                    if 'team_id' in row and 'ncaa_id' not in row:
                        row['ncaa_id'] = row['team_id']
                    teams.append(row)
    ...
    def scrape_teams(self, teams: List[Dict], max_teams: Optional[int] = None) -> List[Player]:
        ...
        for i, team in enumerate(teams_to_scrape, 1):
            ...
            team_id = int(team.get('ncaa_id') or team.get('team_id'))
            team_name = team.get('team', team.get('school_name', ''))
            team_url = team['url']
            # Handle different division column formats
            division = team.get('ncaa_division_formatted', team.get('division', ''))
    ...
    def save_results(self, players: List[Player], division: Optional[str] = None):
        ...
        json_file = self.output_dir / 'json' / f'rosters_mlax_{self.season}{div_suffix}.json'
        csv_file = self.output_dir / 'csv' / f'rosters_mlax_{self.season}{div_suffix}.csv'
    ...
    def _save_error_reports(self, division: Optional[str] = None):
        ...
        if self.zero_player_teams:
            zero_file = self.output_dir / 'csv' / f'rosters_mlax_{self.season}{div_suffix}_zero_players.csv'
        ...
        if self.failed_teams:
            failed_file = self.output_dir / 'csv' / f'rosters_mlax_{self.season}{div_suffix}_failed.csv'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='NCAA Men\'s Lacrosse Roster Scraper',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Scrape all Division I teams
  python src/mlacrosse_roster_scraper.py --division I --season 2025

  # Scrape first 10 Division I teams (testing)
  python src/mlacrosse_roster_scraper.py --division I --limit 10 --season 2025

  # Scrape all teams
  python src/mlacrosse_roster_scraper.py --season 2025

  # Scrape specific team (e.g., LIU)
  python src/mlacrosse_roster_scraper.py --team 593904 --season 2025
        """
    )
    ...
    if args.teams_csv is None:
        if args.division:
            # Try men's specific naming first (2025_I_mens_with_urls.csv)
            args.teams_csv = f'teams/{args.season}_{args.division}_mens_with_urls.csv'
            # Fallback to generic naming if men's specific doesn't exist
            if not Path(args.teams_csv).exists():
                args.teams_csv = f'teams/{args.season}_{args.division}_teams_with_urls.csv'
        else:
            # Default to Division I men's teams
            args.teams_csv = f'teams/{args.season}_I_mens_with_urls.csv'
            if not Path(args.teams_csv).exists():
        ...
        if not Path(args.teams_csv).exists():
            fallback_path = args.teams_csv.replace('_with_urls', '').replace('_mens', '')
